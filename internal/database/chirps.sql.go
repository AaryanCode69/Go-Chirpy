...
const sendChirp = `-- name: SendChirp :one
...
type SendChirpParams struct {
	Body   string
	UserID uuid.UUID
}

func (q *Queries) SendChirp(ctx context.Context, arg SendChirpParams) (Chirp, error) {
	row := q.db.QueryRowContext(ctx, sendChirp, arg.Body, arg.UserID)
	var i Chirp
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
